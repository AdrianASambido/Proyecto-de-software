...
def index(sitio_id):
    ...
    filtros = {}
    if request.args.get("usuario") is not None and request.args.get("usuario") != "" and request.args.get("usuario") != "Todos":
        filtros["usuario"] = request.args.get("usuario")
    if request.args.get("accion") is not None and request.args.get("accion") != "" and request.args.get("accion") != "Todos":
        filtros["accion"] = request.args.get("accion")
    if request.args.get("fecha_desde") is not None and request.args.get("fecha_desde") != "":
        filtros["fecha_desde"] = request.args.get("fecha_desde")
    if request.args.get("fecha_hasta") is not None and request.args.get("fecha_hasta") != "":
        filtros["fecha_hasta"] = request.args.get("fecha_hasta")
    datos_historial=list_site_history(sitio_id, page, filtros if len(filtros.keys()) > 0 else None)
